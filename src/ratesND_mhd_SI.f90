...
!----------------------------------------------------------------------------
!  if ismooth.eq.1, calculate the barycentric velocity until converged
!---------------------------------------------------------------------------- 
...
!
    if (idrag_nature.ne.0 .and. Kdrag.gt.0. .and. ismooth.ne.1) then
...
!--OK    call interpolate_kerneldrag(q2i,wabi)
    wabi     = wabi*hfacwabi
!--DIRTY HACK
   call interpolate_kernel(q2j,wabj,gkerj) 
!-OK    call interpolate_kerneldrag(q2j,wabj)
    wabj     = wabj*hfacwabj
    !wab = 0.5*(wabi + wabj)
    if (itypei.eq.itypegas) then
       wab = wabi
...
!--OK    dragterm    = ndim*wab*dragcoeff*f*V0        
    dragterm_en = dragterm*V0
!--DIRTY HACK    
    forcei(:)   = forcei(:)  - dragterm*pmassj*dvel(:)
    force(:,j)  = force(:,j) + dragterm*pmassi*dvel(:)    
!-OK    forcei(:)   = forcei(:)  - dragterm*pmassj*drdrag(:)   
!-OK    force(:,j)  = force(:,j) + dragterm*pmassi*drdrag(:)
    if (itypei.eq.itypegas) then
