...
!      
 ...
 OPEN(UNIT=iread,ERR=999,FILE=infile,STATUS='old',FORM='formatted')
  READ(iread,NML=inopts)
  IF (imhd.NE.0) READ(iread,NML=mhdopts)
 CLOSE(UNIT=iread)
...
! CLOSE(UNIT=iread)
 OPEN(UNIT=iread,FILE='crap',STATUS='replace',FORM='formatted')
  WRITE(iread,NML=inopts)
  WRITE(iread,NML=mhdopts)
 CLOSE(UNIT=iread)
