...
!
    ...
    do i=0,ikern
       ...
       q = sqrt(q2)
       if (q < 0.4) then
          wkern(i)     = 6.*q2*q2 - 9.6*q2 + 5.888 
          grwkern(i)   = q*(24.*q2 - 19.2) 
          grgrwkern(i) = 72.*q2 - 19.2 
          fsoft(i)     = 125.*q*(1125.*q2*q2 - 2520.*q2 + 2576.)/172032. 
          potensoft(i) = 15625.*q2*q2*q2/114688. - 1875.*q2*q2/4096. + 2875.*q2/3072. - &
                         1199./768. 
          dphidh(i)    = -15625.*q2*q2*q2/16384. + 9375.*q2*q2/4096. - 2875.*q2/1024. + &
                         1199./768. 
       elseif (q < 1.2) then
          wkern(i)     = -5.*(-q + 1.2)**4 + (-q + 2.)**4 
          grwkern(i)   = -16.*q2*q + 48.*q2 - 38.4*q + 2.56 
          grgrwkern(i) = -48.*q2 + 96.*q - 38.4 
          fsoft(i)     = (-234375.*q**7 + 1093750.*q2*q2*q2 - 1575000.*q2*q2*q + 262500.*q2*q2 + &
                         770000.*q2*q + 64.)/(430080.*q2) 
          potensoft(i) = (-78125.*q**7 + 437500.*q2*q2*q2 - 787500.*q2*q2*q + 175000.*q2*q2 + &
                         770000.*q2*q - 1341760.*q - 128.)/(860160.*q) 
          dphidh(i)    = 15625.*q2*q2*q2/24576. - 3125.*q2*q2*q/1024. + 9375.*q2*q2/2048. - &
                         625.*q2*q/768. - 1375.*q2/512. + 599./384. 
       elseif (q < 2.0) then
          wkern(i)     = (-q + 2.)**4 
          grwkern(i)   = -4.*(-q + 2.)**3 
          grgrwkern(i) = 12.*q2 - 48.*q + 48. 
          fsoft(i)     = (46875.*q**7 - 437500.*q2*q2*q2 + 1575000.*q2*q2*q - 2625000.*q2*q2 + &
                         1750000.*q2*q - 55936.)/(344064.*q2) 
          potensoft(i) = (15625.*q**7 - 175000.*q2*q2*q2 + 787500.*q2*q2*q - 1750000.*q2*q2 + &
                         1750000.*q2*q - 1400000.*q + 111872.)/(688128.*q) 
          dphidh(i)    = -15625.*q2*q2*q2/98304. + 3125.*q2*q2*q/2048. - 46875.*q2*q2/8192. + &
                         15625.*q2*q/1536. - 15625.*q2/2048. + 3125./1536. 
       else
          wkern(i)     = 0. 
          grwkern(i)   = 0. 
          grgrwkern(i) = 0. 
...
!
    ...
    select case(ndim)
      case(1)
         cnormk = 243./2560.
      case(2)
         cnormk = 15309./(61184.*pi)
      case(3)
         cnormk = 2187./(10240.*pi)
    end select
    do i=0,ikern
       q2 = i*dq2table
       q = sqrt(q2)
       if (q < 2./3.) then
          wkern(i) = 15.*(-q + 2./3.)**5 - 6*(-q + 4./3.)**5 + (-q + 2.)**5 
          grwkern(i) = -75.*(-q + 2./3.)**4 + 30*(-q + 4./3.)**4 - 5*(-q + 2.)**4 
          grgrwkern(i) = 300.*(-q + 2./3.)**3 - 120*(-q + 4./3.)**3 + 20*(-q + 2.)**3 
       elseif (q < 4./3.) then
          wkern(i) = -6.*(-q + 4./3.)**5 + (-q + 2.)**5 
          grwkern(i) = 30.*(-q + 4./3.)**4 - 5.*(-q + 2.)**4 
          grgrwkern(i) = -120.*(-q + 4./3.)**3 + 20.*(-q + 2.)**3 
       elseif (q < 2.) then
          wkern(i) = (-q + 2.)**5 
          grwkern(i) = -5.*(-q + 2.)**4 
          grgrwkern(i) = 20.*(-q + 2.)**3 
       elseif (2 < q) then
          wkern(i) = 0.
          grwkern(i) = 0. 
          grgrwkern(i) = 0.
...
!
    kernellabel = 'Wendland 1D kernel of degree 6' 
...
!
    kernellabel = 'twice integrated M4' 
...
!
    kernellabel = 'twice integrated M5' 
...
!
    kernellabel = 'triple-integrated M4' 
